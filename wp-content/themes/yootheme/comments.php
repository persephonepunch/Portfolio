...
<div id="comments" class="uk-margin-large">
    ...
    <?php if (have_comments()) : ?>
        ...
        <h2 class="uk-h3 uk-heading-bullet uk-margin-medium-bottom"><?php printf(_n('Comment', 'Comments (%s)', get_comments_number(), 'yootheme'), number_format_i18n(get_comments_number())) ?></h2>
        ...
        <ul class="uk-pagination uk-flex-between">
            <li class="nav-previous"><span uk-pagination-previous></span> <?php previous_comments_link(__('Older Comments', 'yootheme')) ?></li>
            <li class="nav-next"><?php next_comments_link(__('Newer Comments', 'yootheme')) ?> <span uk-pagination-next></span></li>
        </ul>
        <?php endif ?>
    ...
    <?php if (!comments_open() && get_comments_number() && post_type_supports(get_post_type(), 'comments')) : ?>
        <p class="uk-margin-medium uk-text-danger"><?php _e('Comments are closed.', 'yootheme') ?></p>
    <?php endif ?>
...
<li id="comment-<?php comment_ID() ?>">
    <article id="comment-article-<?php comment_ID() ?>" <?php comment_class('uk-comment uk-visible-toggle') ?>>
