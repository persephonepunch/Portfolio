...
$meta = function () use ($params) {
    ...
    $category = $params['categories'] ? get_the_category_list(__(', ', 'yootheme')) : '';
        ...
        switch ($params['meta_style']) {
            ...
            case 'list':
                ...
                <ul class="uk-subnav uk-subnav-divider<?= $params['header_align'] ? ' uk-flex-center' : '' ?>">
                    ...
                    <?php if ($category && count(get_categories()) > 1): ?>
            ...
            default: // sentence
                ...
                if ($category && $categories = get_the_category_list(__(', ', 'yootheme'))) {
...
<article id="post-<?php the_ID() ?>" <?php post_class('uk-article') ?> typeof="Article">
    ...
    <?php if ($params['tags'] && $tags = get_the_tag_list('', __(', ', 'yootheme'))) : ?>
    ...
    <?php if (is_single()) {
        wp_link_pages(['before' => '<div class="uk-margin-medium">' . __('Pages:', 'yootheme') . '<ul class="uk-pagination">', 'after'  => '</ul></div>']);
    } ?>

    <?php if ($edit = get_edit_post_link()) : ?>
    <p>
        <a href="<?= esc_url($edit) ?>"><?= __('<span uk-icon="icon: pencil"></span> Edit', 'yootheme') ?></a>
    </p>
    <?php endif ?>

    <?php if (is_single() && $params['navigation']) : ?>
    <ul class="uk-pagination uk-margin-medium">
        <?php if ($prev = get_previous_post_link('%link', __('<span uk-pagination-previous></span> Previous', 'yootheme'))) : ?>
        <li><?= $prev ?></li>
        <?php endif ?>
        <?php if ($next = get_next_post_link('%link', __('Next <span uk-pagination-next></span>', 'yootheme'))) : ?>
